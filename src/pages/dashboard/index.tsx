        ...
        <div className="space-y-6 lg:space-y-10 font-sans selection:bg-amber-500/30 selection:text-amber-200">
            ...
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
                <div>
                    ...
                    <p className="text-xs sm:text-sm lg:text-base text-zinc-500 dark:text-zinc-400 font-medium">Portföyünüzün genel durumu ve canlı kurlar.</p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-10">
                <div className="xl:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
                    ...
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 transition-all duration-300">
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isDisclaimerExpanded ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                            <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed pl-[3.25rem]">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white/50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-200 dark:border-white/5 backdrop-blur-sm">
                        {/* Tabs */}
                    ...
                    {/* Rates Table */}
                    <div className="bg-white/40 dark:bg-zinc-900/40 border border-zinc-200 dark:border-white/5 rounded-3xl overflow-hidden backdrop-blur-md shadow-2xl relative min-h-[400px]">
                        {isLoading && (
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-zinc-200 dark:divide-white/5">
                                    {filteredRates.map((cur: any) => {
                                        ...
                                        return (
                                            <tr key={cur.id} className="group hover:bg-zinc-100 dark:hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-3 py-3 sm:px-6 sm:py-5 text-center w-12">
                                                    <button 
                                                        ...
                                                        className={`p-2 rounded-full transition-colors ${favoriteUnits?.includes(cur.name) ? 'text-amber-500 bg-amber-500/10' : 'text-zinc-600 hover:text-amber-500 hover:bg-zinc-100 dark:bg-zinc-800'}`}
                                                    >
                                                ...
                                                <td className="px-3 py-3 sm:px-6 sm:py-5 text-right">
                                                    <div className="text-zinc-900 dark:text-white font-bold font-mono text-xs sm:text-base">₺{parseFloat(cur.buying).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</div>
                                                </td>
                                                <td className="px-3 py-3 sm:px-6 sm:py-5 text-right">
                ...
                <div className="space-y-4 sm:space-y-6">
                    <div className="bg-gradient-to-b from-zinc-900/60 to-zinc-900/20 border border-zinc-200 dark:border-white/5 rounded-xl sm:rounded-2xl lg:rounded-[3rem] p-4 sm:p-6 md:p-8 lg:p-10 backdrop-blur-2xl flex flex-col shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-3 sm:space-y-4 md:space-y-6">
                                <div className="p-3 sm:p-4 md:p-6 bg-zinc-100 dark:bg-white/5 rounded-xl sm:rounded-2xl md:rounded-3xl border border-zinc-200 dark:border-white/5">
                                    <p className="text-[9px] sm:text-[10px] text-zinc-500 dark:text-zinc-400 font-black uppercase tracking-widest mb-2 sm:mb-3 md:mb-4">Hızlı Erişim</p>
                                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                                        <button
                                            onClick={() => navigate(PATHS.ASSETS)}
                                            className="flex flex-col items-center gap-1.5 sm:gap-2 md:gap-3 p-2.5 sm:p-3 md:p-4 bg-white dark:bg-zinc-900 rounded-lg sm:rounded-xl md:rounded-2xl border border-zinc-200 dark:border-white/5 hover:bg-zinc-100 dark:hover:bg-white/5 transition-colors"
                                        >
                                            <Wallet className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-amber-500" />
                                            <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400">Varlıklar</span>
                                        </button>
                                        <button
                                        ...
                                        >
                                            ...
                                            <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400">Ayarlar</span>
                                        </button>
                                    </div>
                                </div>
