...
import { logout, setTheme } from '@/features/app';
...
const Sidebar: React.FC = () => {
    ...
    return (
        <aside className={`${isCollapsed ? 'w-16 sm:w-20' : 'w-64 xl:w-72'} border-r border-zinc-200 dark:border-white/5 bg-white/80 dark:bg-zinc-900/30 flex flex-col p-3 sm:p-4 xl:p-6 fixed inset-y-0 flex transition-all duration-300 z-50`}>
                ...
                {isCollapsed ? (
                    <div className="flex flex-col items-center gap-3 sm:gap-4">
                        ...
                        <button
                            ...
                            className="p-1.5 hover:bg-zinc-100 dark:hover:bg-white/5 rounded-lg text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-white transition-all hidden lg:block"
                            title="Genişlet"
                        >
                    ...
                    <div className="space-y-4">
                        ...
                        <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-2 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-white hover:bg-zinc-100 dark:hover:bg-white/5 rounded-lg transition-all text-sm hidden lg:flex"
                            title="Daralt"
                        >
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 xl:gap-4'} ${isCollapsed ? 'px-2 sm:px-3' : 'px-3 xl:px-4'} ${isCollapsed ? 'py-2 sm:py-2.5' : 'py-2.5 xl:py-3'} ${isCollapsed ? 'rounded-lg' : 'rounded-xl xl:rounded-2xl'} transition-all font-medium text-sm xl:text-base ${isActive
                                ? 'bg-zinc-100 dark:bg-white/5 border border-zinc-300 dark:border-white/10 text-amber-500 font-bold shadow-lg shadow-amber-500/5'
            ...
            <button
                onClick={toggleTheme}
                className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 xl:gap-4'} ${isCollapsed ? 'px-2 sm:px-3' : 'px-3 xl:px-4'} ${isCollapsed ? 'py-2 sm:py-2.5' : 'py-2.5 xl:py-3'} ${isCollapsed ? 'rounded-lg' : 'rounded-xl xl:rounded-2xl'} text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-all font-medium mt-auto mb-2 text-sm xl:text-base`}
                title={isCollapsed ? (theme === 'dark' ? 'Aydınlık Mod' : 'Karanlık Mod') : ''}
            >
                {theme === 'dark' ? <Sun className={`${isCollapsed ? 'w-4 h-4 sm:w-5 sm:h-5' : 'w-4 h-4 xl:w-5 xl:h-5'} shrink-0`} /> : <Moon className={`${isCollapsed ? 'w-4 h-4 sm:w-5 sm:h-5' : 'w-4 h-4 xl:w-5 xl:h-5'} shrink-0`} />}
                {!isCollapsed && <span className="truncate">{theme === 'dark' ? 'Aydınlık Mod' : 'Karanlık Mod'}</span>}
            </button>

            <button
                onClick={handleLogout}
                className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 xl:gap-4'} ${isCollapsed ? 'px-2 sm:px-3' : 'px-3 xl:px-4'} ${isCollapsed ? 'py-2 sm:py-2.5' : 'py-2.5 xl:py-3'} ${isCollapsed ? 'rounded-lg' : 'rounded-xl xl:rounded-2xl'} text-zinc-500 dark:text-zinc-400 hover:text-red-400 hover:bg-red-400/10 transition-all font-medium text-sm xl:text-base`}
